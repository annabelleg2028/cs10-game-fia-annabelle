...
# Increased scaling as requested
SPRITE_SCALING_PLAYER = 0.3
MOVEMENT_SPEED = 7
SCROLL_SPEED = 4  # This controls how fast the world moves down
...
class GameView(arcade.View):
    ...
    def create_hazard(self, start_y=None):
        # Use a built-in resource for hazards so the code runs,
        # you can change this to a local path if you have an obstacle image!
        hazard = arcade.Sprite(":resources:images/tiles/spikes90.png", 0.6)
        ...
        # If moving, give it a horizontal speed
    ...
    def on_update(self, delta_time: float) -> None:
        ...
        # Handle Hazards (Move down with background)
        for hazard in self.hazard_list:
            hazard.center_y -= SCROLL_SPEED

            # If it's a moving type, slide left/right
            if not hazard.is_stationary:
                hazard.center_x += hazard.change_x
                if hazard.left < 0 or hazard.right > SCREEN_WIDTH:
                    hazard.change_x *= -1

            # If it falls off the bottom, move it back to the top
            if hazard.top < 0:
                hazard.center_y = SCREEN_HEIGHT + random.randint(50, 200)
                hazard.center_x = random.randint(50, SCREEN_WIDTH - 50)

...
class GameView(arcade.View):
    ...
    def setup(self):
        ...
        # Initial Spawning: Massive vertical gaps (450px)
        for i in range(4):
            self.create_hazard(start_y=SCREEN_HEIGHT + (i * 450))
            if i % 2 == 0:
                self.create_token(start_y=SCREEN_HEIGHT + (i * 450) + 225)

    def get_safe_position(self, start_y):
        """Finds a spot with a massive 350px buffer."""
        max_attempts = 100
        for _ in range(max_attempts):
            ...
            # Distance check against hazards
    ...
    def on_update(self, delta_time: float) -> None:
        ...
        for hazard in self.hazard_list:
            ...
            if hazard.top < 0:
                hazard.center_x, hazard.center_y = self.get_safe_position(SCREEN_HEIGHT + 400)
        ...
        for token in hits:
            ...
            token.center_x, token.center_y = self.get_safe_position(SCREEN_HEIGHT + 600)
            token.value = random.choice([1, 1, 5, 5, 10, -5])
